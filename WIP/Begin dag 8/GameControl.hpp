...
private:
    ...
    std::map<std::string, picture> objectList = {
		...
		{"Quit_GameButton", picture( sf::Vector2f( 0.0, 800.0 ), "images/Quit.png", sf::Vector2f( 380.0, 70.0 ))}
    };
...
public:
    ...
    void update(){
        ...
        for(auto & enemy: enemyList){
            enemy->move();
            if(enemy->shoot()){
                picture tmp = picture(enemy->getTexture().getPosition(), "images/lazer_green.png", sf::Vector2f(10.0, 25.0), "enemyBullet1" );
        ...
        for(auto & bullet: enemyBulletList){
            bullet.move(sf::Vector2f{ 0.0, 5.0 });
